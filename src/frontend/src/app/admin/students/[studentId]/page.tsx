...
export default async function AdminStudentDetailPage({
  ...
  const detail = getAdminStudentLeakDetail(dashboardData, studentId);
          ...
          <AdminPanel tone="hero" className="px-7 py-7">
            <div className="grid gap-6 2xl:grid-cols-[minmax(0,1.3fr)_minmax(340px,0.7fr)]">
              ...
              <AdminPanel tone="feature" className="px-5 py-5">
                ...
                <AdminTextBlock label="교사용 해석" tone="emphasis">
                  {detail.teacherFocus}
                </AdminTextBlock>
          ...
          <AdminMotionSection delay={0.06} className="min-w-0">
            <AdminPanel className="px-6 py-6">
              <AdminSectionTitle
                ...
                title="학생이 놓친 연결 고리"
                description="사건을 다시 나열하는 대신, 어떤 흐름이 어디서 끊겼는지와 시험 축을 함께 남깁니다."
              />
                ...
                {detail.brokenChains.map((chain, index) => (
                  ...
                  >
                    ...
                    <div className="mt-4 grid gap-3 xl:grid-cols-[minmax(0,1.1fr)_minmax(220px,0.9fr)]">
                      <AdminTextBlock label="왜 끊기나">
                        {chain.whyItBreaks}
                      </AdminTextBlock>
                      <AdminValueBox
                        label="시험 축"
                        value={chain.examAxis}
                        detail="시험에서 같이 설명돼야 하는 연결"
                      />
                    </div>
                  </article>
                ))}
            ...
            <AdminMotionSection delay={0.1}>
              <AdminPanel tone="soft" className="px-5 py-5">
                <AdminSectionTitle
                  ...
                  title="최근 복습 신호"
                  description="학생 화면과 연결되는 복습 흔들림을 오른쪽에서 바로 확인합니다."
                />
            ...
            <AdminMotionSection delay={0.14}>
              <AdminPanel tone="soft" className="px-5 py-5">
                <AdminSectionTitle
                  ...
                  title="수업 메모"
                  description="실제 설명 전략과 학생 반응을 기록층처럼 차분하게 정리합니다."
                />
        ...
        <AdminMotionSection delay={0.18}>
          <AdminPanel tone="soft" className="px-6 py-6">
            <AdminSectionTitle
              ...
              title="수업 개입 이력"
              description="개별 설명과 복습 배정이 실제로 어떤 결과를 만들었는지 시간순으로 남깁니다."
              action={
                <AdminFrameLabel icon={<Clock3 size={13} />} text={`${detail.interventions.length}건`} subtle />
              }
            />
